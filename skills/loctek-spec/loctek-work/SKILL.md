...
- 用户关键决策：范围、取舍、产品口径、验收口径。
- 实现思路：为什么这样改。
- 放弃方案：考虑过但没有采用的方案。
- 合并时必须保留：后续 merge/rebase 不能丢的行为。
...
如果用户没有显式使用 `loctek-work`，项目级 AI 规则仍应把关键决策写进 `.changes/session-notes/`。当你正在维护 work report 时，优先写 work report，避免重复写两处。
